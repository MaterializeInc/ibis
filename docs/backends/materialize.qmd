...
def print_table(all_bids, batch_num, total_updates):
    ...
    # Header
    print("=" * 100)
    print("🔨 LIVE AUCTION MONITOR - Powered by Ibis + Materialize")
    print("=" * 100)
    print(f"\n📊 Refresh #{batch_num} | Active Auctions: {len(all_bids)}")
    print(f"⏰ Time: {datetime.now().strftime('%H:%M:%S')}\n")

    # Table header
    print("🏆 ALL ACTIVE AUCTIONS - WINNING BIDS (Sorted by Highest Bid)")
    print("-" * 100)
    ...
    print("-" * 100)
    if len(all_bids) > 15:
        print(f"💡 Showing top 15 of {len(all_bids)} active auctions by bid amount")
    else:
        print(f"💡 Showing all {len(all_bids)} active auctions")
